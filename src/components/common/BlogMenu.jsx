...
const BlogMenu = ({ activesMenus }) => {
  ...
  return (
    <li
      className={
        activesMenus === '/blog-list' ||
        activesMenus === '/blog-grid' ||
        activesMenus === '/blog-details' ||
        activesMenus === '/blog-list-sidebar' ||
        activesMenus === '/blog-grid-sidebar'
          ? 'has-submenu active'
          : 'has-submenu'
      }
    >
      <Link to='' className={menu5 ? 'submenu' : ''}>
        Blog{' '}
        <i className='fas fa-chevron-down' onClick={() => setMenu5(!menu5)}></i>
      </Link>
